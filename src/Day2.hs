...
solvePart1 = length . filter valid
    where valid (Password range letter password) = inRange range . length . filter (== letter) $ password

solvePart2 :: [Password] -> Int
solvePart2 = length . filter valid
    where valid (Password (a, b) letter password) = (password !! (a - 1) == letter) /= (password !! (b - 1) == letter) 

readPasswordFile :: FilePath -> IO [Password]
readPasswordFile file = fmap readPassword . lines <$> readFile file
    where readPassword input = let 
